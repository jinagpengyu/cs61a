...
def tail_points(opponent_score):
    ...
    if opponent_score == 0 : 
        return 1 
    if opponent_score > 0 and opponent_score < 10 : 
...
def is_always_roll(strategy, goal=GOAL):
    ...
    score1 = 0 
    score2 = 0
    strat = strategy(score1,score2)
    while score2 < goal : 
        score1 = score1 + 1
        end = strategy(score1,score2)
        if strat != end : 
            return False 
        if score1 == goal :
            score2 = score2 + 1
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 10
    num = num_rolls 
    tail_num = tail_points(opponent_score)
    if tail_num >= threshold : 
        num = 0
    return num 
    # END PROBLEM 10
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    num = num_rolls 
    tail_num = tail_points(opponent_score)
    new_score = tail_num + score 
    #print(new_score)
    if perfect_square(new_score) == True :
